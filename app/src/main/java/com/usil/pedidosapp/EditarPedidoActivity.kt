...
class EditarPedidoActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            PedidosAPPTheme {
                PedidoMainView(taskId = taskId, viewModel = pedidoViewModel)
            }
        }
    }
}
...
@Preview
@Composable
fun PedidoMainView(taskId: Int = -1, viewModel: PedidoViewModel = viewModel()) {
    ...
    LaunchedEffect(Unit) {
        if (isEditMode) {
            ...
            viewModel.getPedidoById(taskId)
        }
    }

    LaunchedEffect(selectedPedido) {
        selectedPedido?.let { todo ->
            /*taskName = todo.name
    ...
    Scaffold(
        ...
        content = { paddingValues ->
            FormUpsertView(
                ...
                onSaveTask = {
                    //viewModel.createPedido(taskName, isCompleted)
                },
                onUpdateTask = {
